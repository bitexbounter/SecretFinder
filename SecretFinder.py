...
#  Modified By Bitex github.com/bitexbounter
...
_regex = {
    ...
    # SSH Private Key
    'SSH_privKey': r '-----BEGIN OPENSSH PRIVATE KEY-----',
    
    # Google Cloud Service Account Key
    'google_service_account_key': r'\"type\":\s*\"service_account\"',
    
    # Google API Client Secret
    'google_api_client_secret': r'\"client_secret\":\s*\"[0-9A-Za-z-_]{32}\"',
    
    # Microsoft Azure Subscription Key
    'azure_subscription_key': r'(?i)Subscription\s*Key:\s*[0-9a-f]{32}',
    
    # Microsoft Azure Client Secret
    'azure_client_secret': r'\"client_secret\":\s*\"[0-9a-zA-Z]{32}\"',
    
    # GitLab Access Token
    'gitlab_access_token': r'glpat-[0-9a-zA-Z]{20,40}',
    
    # DigitalOcean API Token
    'digitalocean_api_token': r'(?i)do_token\s*=\s*\"[0-9a-f]{64}\"',
    
    # MongoDB Connection String
    'mongodb_connection_string': r'mongodb://[a-zA-Z0-9._%+-]+:[^@]+@[^/]+/[a-zA-Z0-9._%+-]+',
    ...
    # Twilio Auth Token
    'twilio_auth_token': r'(?i)TWILIO_AUTH_TOKEN\s*=\s*\"[0-9a-zA-Z]{32}\"',
    
    # SendGrid API Key
    'sendgrid_api_key': r'SG\.[A-Za-z0-9_-]{22}\.[A-Za-z0-9_-]{43}',
    
    # Auth0 Client Secret
    'auth0_client_secret': r'(?i)client_secret\s*=\s*\"[0-9a-zA-Z]{32}\"',
    
    # Okta API Token
    'okta_api_token': r'(?i)api_token\s*=\s*\"[0-9a-zA-Z]{32}\"',
    
    # Box API Key
    'box_api_key': r'(?i)box_api_key\s*=\s*\"[0-9a-zA-Z]{32}\"',
    
    # Mailchimp API Key
    'mailchimp_api_key': r'(?i)api_key\s*=\s*\"[0-9a-zA-Z]{32}-[0-9a-zA-Z]{4}\"',
    
    # Slack Webhook URL
    'slack_webhook_url': r'https:\/\/hooks\.slack\.com\/services\/T[0-9A-Z]{8}\/B[0-9A-Z]{8}\/[0-9A-Za-z]{24}',
    
    # GitHub OAuth Token
    'github_oauth_token': r'ghp_[0-9a-zA-Z]{36}',
    
    # Google Maps API Key
    'google_maps_api_key': r'AIza[0-9A-Za-z-_]{35}',
    
    # API Gateway Key
    'api_gateway_key': r'(?i)x-api-key:\s*[0-9a-zA-Z]{32}',
    
    # Cloudflare API Token
    'cloudflare_api_token': r'(?i)api_token\s*=\s*\"[0-9a-zA-Z]{32}\"',
    
    # Heroku API Key
    'heroku_api_key': r'(?i)heroku_api_key\s*=\s*\"[0-9a-f]{32}\"',
    
    # Firebase Auth Token
    'firebase_auth_token': r'eyJ[0-9A-Za-z-_]{3,}\.[0-9A-Za-z-_]{3,}\.[0-9A-Za-z-_]{3,}',
    
    # AWS Secret Access Key
    'aws_secret_access_key': r'(?i)AKIA[0-9A-Z]{16}',
    
    # AWS Session Token
    'aws_session_token': r'(?i)[A-Za-z0-9+/=]{128}',
    
    # API Key in URL
    'api_key_in_url': r'(?i)(api_key|access_token|token|key)=[0-9a-zA-Z-_]{32,}',
    
    # OAuth 2.0 Token
    'oauth2_token': r'ey[A-Za-z0-9-_=]+\.[A-Za,
}
